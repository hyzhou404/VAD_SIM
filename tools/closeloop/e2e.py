...
from visualizer import save_frame, to_video
...
def main():
    ...
    cnt = 0
    vis_process_pool = []
    output_folder = os.path.join(args.output, 'vad')
    ...
    while True:
        ...
        results[0]['pts_bbox']['command'] = torch.argmax(data['ego_fut_cmd'][0,0,0]).cpu().detach().numpy()
        
        save_fn = osp.join(output_folder, f'{str(cnt).zfill(4)}')
        save_frame(raw_images, plan_traj, raw_data[1]['cam_params'], save_fn)
